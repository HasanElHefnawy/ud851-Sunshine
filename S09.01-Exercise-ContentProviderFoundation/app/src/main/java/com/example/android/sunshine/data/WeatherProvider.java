...
//  TODO (6) Write a method called buildUriMatcher where you match URI's to their numeric ID
    public UriMatcher buildUriMatcher() {
        ...
        uriMatcher.addURI(CONTENT_AUTHORITY, TABLE_NAME, CODE_WEATHER);
        uriMatcher.addURI(CONTENT_AUTHORITY, TABLE_NAME + "/#", CODE_WEATHER_WITH_DATE);
        return uriMatcher;
    }
...
//      TODO (9) Handle queries on both the weather and weather with date URI
        ...
        switch (match) {
            case CODE_WEATHER:
                // Query employee table all row data.
                cursor = db.query(TABLE_NAME, projection, selection, selectionArgs, null, null, sortOrder);
                break;
            case CODE_WEATHER_WITH_DATE:
                // Query employee table single row.
