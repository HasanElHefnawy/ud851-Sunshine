...
//      TODO (7) Generate a weather summary with the date, description, high and low
        long dateLong = mCursor.getLong(mCursor.getColumnIndex(COLUMN_DATE));
        int high = mCursor.getInt(mCursor.getColumnIndex(COLUMN_MAX_TEMP));
        int low = mCursor.getInt(mCursor.getColumnIndex(COLUMN_MIN_TEMP));
        String weatherForThisDay = SunshineDateUtils.getFriendlyDateString(mContext, dateLong, true) + " " + high + " " + low;
